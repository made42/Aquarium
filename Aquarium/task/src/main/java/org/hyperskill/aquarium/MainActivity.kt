...
class AnimalsAdapter(val imageAnimals: List<String>, val nameAnimals: List<String>, val descriptionAnimals: List<String>) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        Picasso.get().load(imageAnimals[position]).placeholder(R.drawable.placeholder).error(R.drawable.error).into(holder.imageView);
...
class MainActivity : AppCompatActivity() {
    ...
    private var imageAnimals = listOf(
    ...
    private var nameAnimals = listOf("Koi Carp", "Spiny Dogfish", "Kaluga");
    private var descriptionAnimals = listOf(
        //Koi Carp
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.extras?.get("imageAnimals") != null) {
            imageAnimals = intent.extras?.getSerializable("imageAnimals")   as List<String>
        }
        if (intent.extras?.get("nameAnimals") != null) {
            nameAnimals = intent.extras?.getSerializable("nameAnimals") as List<String>
        }
        if (intent.extras?.get("descriptionAnimals") != null) {
            descriptionAnimals = intent.extras?.getSerializable("descriptionAnimals") as List<String>
        }

        viewpager2.adapter = AnimalsAdapter(imageAnimals, nameAnimals, descriptionAnimals)
